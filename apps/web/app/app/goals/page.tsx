...
function standingButtonLabel(s: Standing) {
  return `הוראת קבע · ${formatIls(s.monthlyAmount)} בחודש`;
}
...
function GoalsInner() {
  ...
  const [title, setTitle] = useState("קרן חירום");
  ...
  async function commitConfirm() {
    ...
    try {
      if (confirm.mode === "stop-standing") {
        ...
        setMsgHref(
          `/app/money?month=${confirm.fromMonth || month}&category=goal_funding`,
        );
      } else {
        const res = await api<{
          messageHe?: string;
        ...
        setMsgHref(
          `/app/money?month=${res.month || confirm.month}&category=goal_funding`,
        );
  ...
  async function saveStandingPanel() {
    ...
    try {
      ...
      const g = refreshed.find((x) => x.id === standingPanel.goalId);
      if (g?.standing) {
  ...
  const poolTotal = Math.max(
    pool?.poolBase || 1,
    (pool?.allocated || 0) + (pool?.plannedStanding || 0) + (pool?.free || 0),
    1,
  );
  ...
  return (
    <div className="grid" style={{ gap: "0.85rem" }}>
      <PageHeader
        title="יעדים"
        subtitle="הקצאה מהנותר לחודש הנבחר — ידנית או בהוראת קבע"
        actions={
          <button
            className="btn secondary"
            type="button"
        ...
        extra={
          pool ? (
            <span>
              ...
              {" · "}
              פנוי <strong>{formatIls(pool.free)}</strong>
            </span>
          ) : null
        }
      />
      ...
      {pool && (
        <section className="card month-pool">
          <h2 style={{ marginTop: 0, marginBottom: "0.5rem" }}>
            הנותר של {pool.labelHe}
          </h2>
          <div className="month-pool-bar" aria-hidden>
            <div
              className="month-pool-seg allocated"
              style={{
                width: `${Math.min(100, (pool.allocated / poolTotal) * 100)}%`,
              }}
            />
            <div
              className="month-pool-seg planned"
              style={{
                width: `${Math.min(100, (pool.plannedStanding / poolTotal) * 100)}%`,
              }}
            />
            <div
              className="month-pool-seg free"
              style={{
                width: `${Math.min(100, (pool.free / poolTotal) * 100)}%`,
              }}
            />
          </div>
          <p className="muted" style={{ margin: "0.45rem 0 0", fontSize: "0.85rem" }}>
            נותר {formatIls(pool.leftover)} · בקבע{" "}
            {formatIls(pool.plannedStanding)} · הוקצה {formatIls(pool.allocated)}{" "}
            · פנוי {formatIls(pool.free)}
            {pool.tight
              ? " · הקבע גדול מהפנוי — בחרו למי להקצות קודם"
              : ""}
          </p>
        </section>
      )}
      ...
      <section className="card">
        <h2 style={{ marginTop: 0 }}>היעדים שלי</h2>
        {goals.length === 0 && (
          <p className="muted">
            אין יעדים לחודש הנבחר — יעדים שנוצרו מאוחר יותר יופיעו מחודש היצירה
            ואילך.
          </p>
        )}
        {goals.map((g) => {
          ...
          const savedLine =
            remainingAsOf <= 0
              ...
              : `נחסכו ${formatIls(saved)} מתוך ${formatIls(target)}`;
          const pctLabel =
            remainingAsOf <= 0
              ? null
              : pctRaw < 1
          ...
          return (
            <div key={g.id} className="goal-row">
              <div className="goal-card-head">
                ...
                <p className="goal-saved">{savedLine}</p>
                ...
                {pctLabel && (
                  <p className="goal-pct muted">{pctLabel}</p>
                )}
              </div>

              <div className="goal-standing-quiet">
                {!g.standing ? (
                  <button
                    className="btn secondary"
                    type="button"
                  ...
                  <div className="goal-standing-block">
                    <div className="goal-standing-actions">
                      <button
                        ...
                        onClick={() => setStandingPanel(panelFromStanding(g))}
                      >
                        {standingButtonLabel(g.standing)}
                      </button>
                      {monthStatus && (
                        <span
                          className={
                            monthStatus === "done"
                              ? "badge goal-month-done"
                              : "badge"
                          }
                        >
                          {monthStatus === "done"
                            ? `✓ אושר ל${labelMonthHe(month)}`
                            : monthStatus}
                        </span>
                      )}
                    ...
                    <p className="goal-standing-span muted">
                ...
                <div className="goal-allocate">
                  {allocateOpenId !== g.id ? (
                    <button
                      ...
                      type="button"
                      disabled={busy}
                      onClick={() => setAllocateOpenId(g.id)}
                    >
                      הקצאה ידנית
                    </button>
                  ) : (
